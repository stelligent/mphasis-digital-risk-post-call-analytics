...
import { Button, ContentLayout, Spinner, Header, Grid, Container, SpaceBetween } from '@cloudscape-design/components';
...
function Dashboard({ setAlert }) {
  ...
  }, [speakerLabels, isTranscribeCallAnalyticsMode, data])
  ...
  const mitelMetadataColumn = [
    {
      label: "Direction",
      value: (d) => d?.MitelMetadata?.callDirection,
    },
    {
      label: "Agent Name",
      value: (d) => d?.MitelMetadata?.agentName,
    },
    {
      label: "Agent Extension",
      value: (d) => d?.MitelMetadata?.agentExtension,
    },
    {
      label: "Caller Number",
      value: (d) => d?.MitelMetadata?.callerNumber,
    },
    {
      label: "Call Start Time",
      value: (d) => d?.MitelMetadata?.startTime,
    },
    {
      label: "Call End Time",
      value: (d) => d?.MitelMetadata?.endTime,
    },
    {
      label: "Organization",
      value: (d) => d?.MitelMetadata?.organization,
    },
  ]
    ...
    <Grid
      ...
      >
        <Container
          ...
          header={
            <Header variant="h2">
              Digital Risk Call Info
            </Header>
          }
        >
          <SpaceBetween size="m">
            {mitelMetadataColumn.map((entry, j) => (
